...
def get_image_base64(path, name=None):
    ...
    # 拡張子に合わせて base64 形式を作る内部関数（超軽量化・圧縮版）
    def _to_data_uri(file_path):
        try:
            # 画像を読み込み、スマホ送信用に極限まで軽くする
            with Image.open(file_path) as img:
                # 画面表示の最大サイズ(140x240)に合わせて縮小
                img.thumbnail((140, 240))
                buffered = BytesIO()
                
                # 透過画像以外はJPEGに変換してさらにデータ量を削る
                if img.mode in ("RGBA", "P"):
                    img.save(buffered, format="PNG", optimize=True)
                    mime = "image/png"
                else:
                    img = img.convert("RGB")
                    img.save(buffered, format="JPEG", quality=60)  # 画質を落として軽量化
                    mime = "image/jpeg"
                    
                img_str = base64.b64encode(buffered.getvalue()).decode('utf-8')
                return f"data:{mime};base64,{img_str}"
        except Exception:
            # 万が一圧縮に失敗した場合は元の処理で返す
...
def render_image_html(img_src):
    return f'''
    <div class="card-img-wrapper" style="width: 100%; max-width: 140px; aspect-ratio: 140 / 240; margin: 0 auto 5px auto; overflow: hidden; border-radius: 10px; box-shadow: 0 4px 8px rgba(0,0,0,0.3); background-color: #333;">
        <img src="{img_src}" loading="lazy" style="width: 100%; height: 100%; object-fit: cover; display: block;">
    </div>
    '''
...
def render_image_gallery(cards_list):
    ...
    for card in cards_list:
        ...
        img_tag = f'<img src="{src}" loading="lazy">' if src else '<div style="width:100%; aspect-ratio: 140 / 240; background:#333; border-radius:8px;"></div>'
...
if not st.session_state.cards_db:
    with st.spinner("⏳ 初回アクセス中: 全ページの画像とデータベースを準備しています。完了するまで操作せずにお待ちください..."):
        ...
        if st.button("✨ 画像を解析する", type="primary"):
            with st.spinner("画像を解析中..."):
                ...
                if not final_rects:
                    ...
                    # 検出結果の可視化
                    st.image(cv2.cvtColor(debug_img, cv2.COLOR_BGR2RGB), caption="🔴 赤枠が認識されたカード範囲です")

                    if detected_cards_list:
                        st.success(f"解析成功！ {len(detected_cards_list)}枚のカードを特定しました。")
...
with tab_database:
    ...
    if len(st.session_state.cards_db) > 0:
        ...
        # 👇 データベースの一覧を描画する間、スピナーを表示する
        with st.spinner("📊 データベースの一覧を構築しています... しばらくお待ちください ⏳"):
            if view_mode == "📋 表で見る":
...
with tab_build:
    ...
    for c in st.session_state.cards_db:
        name = c["name"]
        main = st.session_state.custom_main_genres.get(name, c.get("main_genre", ""))
        sub = st.session_state.custom_subgroups.get(name, c.get("default_sub", ""))
        tags = st.session_state.custom_tags.get(name, [])
    ...
    # フィルタリング
    # 👇【追加】検索条件が1つでも指定されているかをチェック
    is_filtered = bool(q_name or q_main or q_sub or q_tag)

    if not is_filtered:
        # 何も指定されていない（初期状態）のときは、促すテキストを出して画像を非表示にする
        st.info("👆 上の検索欄から「大分類」「小分類」「タグ」のいずれかを選択（または名前を入力）して絞り込んでください。")
        f_res = pd.DataFrame()  # 空のデータにして下で表示させない
    else:
        f_res = df_build
    ...
    if not f_res.empty:
        # 👇【追加】画像の準備が終わるまで、ぐるぐる回るローディング注意書きを出す
        with st.spinner("画像を読み込んでいます... 完了するまで操作せずにお待ちください ⏳"):
            # 大分類のループ（固定順）
                ...
                for i in range(0, len(sub_items), 8):
                    ...
                    for j, card in enumerate(sub_items[i:i+8]):
                        with cols[j]:
                            ...
                            img_html = render_image_html(get_image_base64(card["path"], name))
                            label_html = f"<div class='card-label'>{name}</div>"
                            st.markdown(img_html + label_html, unsafe_allow_html=True)
                            ...
                            st.button("➕" if can_add else "×", key=f"bl_{main_val}_{sub_val}_{name}_{i}_{j}", 
...
with tab_update:
    ...
    if uploaded_files:
        if not db_hashes:
            ...
            # 👇 アップロードされた画像を1枚ずつ判定している間にスピナーを出す
            with st.spinner("🤖 AIがカードを判定しています... そのままお待ちください ⏳"):
