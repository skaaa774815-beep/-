# -*- coding: utf-8 -*-
"""
Created on Mon Apr 20 00:57:08 2026
...
def render_image_html(img_src):
    return f'''
    <div style="width: 100%; max-width: 140px; aspect-ratio: 140 / 240; margin: 0 auto 5px auto; overflow: hidden; border-radius: 10px; box-shadow: 0 4px 8px rgba(0,0,0,0.3); background-color: #333;">
...
def render_image_gallery(cards_list):
    ...
    for card in cards_list:
        ...
        img_tag = f'<img src="{src}">' if src else '<div style="width:140px; height:240px; background:#333; border-radius:10px;"></div>'
                ...
                if not final_rects:
                    ...
                    if detected_cards_list:
                        ...
                        for name in detected_cards_list:
                            ...
                            display_detected.append({"name": name, "path_or_url": card_info["path_or_url"] if card_info else ""})
