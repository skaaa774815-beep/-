# -*- coding: utf-8 -*-
"""
Created on Mon Apr 20 00:04:20 2026
...
def render_image_gallery(cards_list):
    html = """
    <style>
    .responsive-gallery { display: grid; grid-template-columns: repeat(auto-fill, minmax(80px, 1fr)); gap: 10px; margin-bottom: 20px; }
    .gallery-item { display: flex; flex-direction: column; align-items: center; }
    .gallery-item img { width: 100%; max-width: 105px; aspect-ratio: 105 / 180; object-fit: cover; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.3); transition: transform 0.2s; }
    .gallery-item img:hover { transform: scale(1.05); }
    .gallery-item-title { text-align: center; height: 3.5em; line-height: 1.4; overflow: hidden; font-size: 0.85em; margin-top: 8px; word-break: break-word; }
    </style>
    <div class="responsive-gallery">
    """
    for card in cards_list:
        img_src = get_image_base64(card["path"], card.get("name"))
...
with st.sidebar:
    ...
    st.write("バグや検出違い（カードが認識されない等）があれば、DMに写真（スクショ）付きで送付をお願いします！🙇‍♂️")
    
    # URLの「あなたのTwitterID」の部分をご自身のID（@以降）に書き換えてください
    st.markdown(
        """
        <a href="https://twitter.com/S_Ka774" target="_blank" style="text-decoration: none;">
            <div style="background-color: #000; color: #fff; padding: 8px 12px; border-radius: 5px; text-align: center; font-weight: bold; margin-top: 5px;">
                𝕏 (Twitter) でDMを送る
            </div>
        </a>
        """,
...
with tab_build:
    ...
    if not filtered_db: st.warning("条件に一致するカードがありません。")
    else:
