# -*- coding: utf-8 -*-
"""
Created on Sun Apr 19 22:59:49 2026
    ...
    if os.path.exists("card_images"):
        for filename in os.listdir("card_images"):
            if filename.lower().endswith((".png", ".jpg", ".jpeg")):
                name = os.path.splitext(filename)[0] # 拡張子を除いた名前
                # まだ登録されていない新しいカードなら追加
                if name not in seen_names and name not in exclude_exact and not any(k in name for k in exclude_keywords):
                    db.append({
                        "name": name,
...
def get_image_base64(path, name=None):
    ...
    if name:
        for ext in [".png", ".PNG", ".jpg", ".JPG", ".jpeg", ".JPEG"]:
            target_path = os.path.join("card_images", f"{name}{ext}")
